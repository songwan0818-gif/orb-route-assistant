...
function run(data,done){setBusy(true);if(worker)worker.terminate();try{worker=new Worker('worker.js?v=03');worker.onmessage=function(e){worker.terminate();worker=null;setBusy(false);if(e.data.error)say(e.data.error);else done(e.data.result);};worker.onerror=function(){worker.terminate();worker=null;setBusy(false);say('当前浏览器无法运行分析。请用系统浏览器打开或更新浏览器。');};worker.postMessage(data);}catch(e){setBusy(false);say('浏览器不支持后台分析，请更新浏览器后重试。');}}
...
run({kind:'photo',frame:sourceFrame,corners:q},function(result){var corrected=el('photo').getContext('2d').createImageData(result.frame.width,result.frame.height);corrected.data.set(result.frame.data);frame=corrected;accept(result.recognition);el('crop').open=false;say(result.recognition.fail?'已校正照片。请修正问号格；若较多，请重新选择四角或避免反光重拍。':'照片已校正，30 格已识别。核对后即可规划最多 12 步的路线。');el('edit').scrollIntoView({block:'start'});});};
el('cancel').onclick=function(){loadId++;if(worker)worker.terminate();worker=null;setBusy(false);say('已取消，可重新开始。');};el('solve').onclick=function(){if(board.indexOf(6)!==-1||busy)return;invalidate();say('正在分析路线…');run({kind:'solve',board:board,mode:el('mode').value},function(r){if(!r){say('12 步内未找到消除路线，请核对棋盘或选择更多连击重新分析。');return;}if(r.path.length>13){say('路线超过 12 步，请刷新网页后重试。');return;}route=r;step=0;el('score').textContent='预计 '+r.total+' 连击 · 移动 '+(r.path.length-1)+' 步';el('result').hidden=false;showStep();say('路线已生成，先练习并记住路线，再回到游戏。');el('result').scrollIntoView();});};
function showStep(){
if(!route)return;
